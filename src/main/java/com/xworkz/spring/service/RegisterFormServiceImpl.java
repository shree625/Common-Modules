		...
		try {
			...
			if (isValid == 0) {
				...
				if (emailCount == 0 && phoneCount == 0) {
					...
					logger.info("Encryption of password started");
					/*
					 * //String randomPassword = RandomStringUtils.randomAlphanumeric(8); String
					 * encrptingPassword = EncrptyDecrypt.encrypt(generatedPassword);
					 * personalInfoEntity.setPassword(encrptingPassword);
					 * //personalInfoEntity.setPassword(encrptingPassword);
					 */
					...
					// logger.info("The password is set to encrypted" + encrptingPassword);
					// personalInfoEntity.setPassword(encrptingPassword);
					...
					MailDTO mailDTO = new MailDTO("Anusree", registrationDto.getEmailId(), "Registration Confirmed",
